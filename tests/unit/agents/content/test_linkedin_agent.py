...
LLM_RESPONSE = '{"content": "Great article about AI!\\n\\nRead more now.", "hashtags": ["#AI", "#Marketing"]}'
...
def _mock_db(found=True):
    ...
    prompt_row.__getitem__ = lambda self, i: ["Write LinkedIn post for ARTICLE_TITLE KEYWORD ARTICLE_BODY"][i]
...
async def test_missing_article_id_fails():
    from agents.content.linkedin_agent import LinkedInAgent
    db = _mock_db()
    agent = LinkedInAgent()
...
async def test_output_has_uuid():
    ...
    import uuid
    db = _mock_db()
    result = await LinkedInAgent().execute(_ctx(db))
    uuid.UUID(result.data["linkedin_post_id"])  # raises if invalid


@pytest.mark.asyncio
async def test_output_has_article_id():
    from agents.content.linkedin_agent import LinkedInAgent
    db = _mock_db()
    result = await LinkedInAgent().execute(_ctx(db))
...
async def test_db_insert_called():
    from agents.content.linkedin_agent import LinkedInAgent
    db = _mock_db()
    await LinkedInAgent().execute(_ctx(db))
    calls = [str(c.args[0]) for c in db.execute.call_args_list if "linkedin_posts" in str(c.args[0])]
    assert len(calls) >= 1
...
@pytest.mark.asyncio
async def test_fallback_on_plain_text_response():
    from agents.content.linkedin_agent import LinkedInAgent
    db = _mock_db()
    db_orig_execute = db.execute
    llm = MagicMock()
    llm.complete = AsyncMock(return_value="Just plain text response no JSON")
    ctx = AgentContext(org_id=ORG_ID, run_id="run-1", params={"article_id": ARTICLE_ID}, config={}, db=db, llm=llm)
    result = await LinkedInAgent().execute(ctx)
    assert result.status == "success"
    assert result.data["word_count"] > 0


@pytest.mark.asyncio
async def test_word_count_in_output():
    from agents.content.linkedin_agent import LinkedInAgent
    db = _mock_db()
    result = await LinkedInAgent().execute(_ctx(db))
    assert result.data["word_count"] > 0
